...
| C20：SFT + PPO | 10% (1/10) | 10% (1/10) | 0% | 0% |

可复现的原始日志汇总由 `scripts/collect_task9_ablation.py` 生成到 `results/task9_ablation.csv`。

![四组模型在两个固定 reset window 的 `success_once` 对比](../assets/figures/task9_ablation_success_rate.png)

## 结论与解释

在正确映射的数据和固定评测协议下，20 条严格匹配的专项专家演示带来了最清晰的提升：B20 从 A0 的 30% 提升到已见 70%、未见 80%。A1 的 PPO 则在已见窗口上升到 50%，但未见窗口下降到 20%，提示在线优化可能更贴合训练时遇到的状态，而未必自动提高泛化。

更关键的是，C20 给出了一个不能回避的负向消融：从 B20 的 SFT checkpoint 出发，再用相同任务进行 10 epoch、8 环境的 PPO 后训练，两个独立窗口都降至 10% (1/10)，且 `success_at_end=0%`。因此，本项目最终选择 **B20 而不是 C20** 作为视频展示和可用策略。这个现象说明当前小样本 PPO 超参数与奖励/探索设置没有保持 SFT 策略，可能发生策略漂移或过度更新；它并不证明 PPO 对所有机器人任务都无效。

这正是消融设计的价值：若只展示 C20 的训练中间奖励、或只挑 A1 的 seen 窗口，容易得出“PPO 有效”的不完整结论。两组固定的独立评测显示，本配置下任务匹配的数据质量比继续在线更新更重要。
