...
所有组别均已在两个 reset 窗口分别评测。最终报告只填写实际日志产生的结果：在本次固定的小样本 PPO 配置中，C20 反而低于 B20，这个负向结果被保留而没有剔除。

| 组别 | `success_once`：0–9 | `success_once`：10–19 |
| --- | ---: | ---: |
| A0：基础 π0.5 | 30%（3/10） | 30%（3/10） |
| A1：仅 PPO | 50%（5/10） | 20%（2/10） |
| B20：仅 20-demo SFT | **70%（7/10）** | **80%（8/10）** |
| C20：SFT + PPO | 10%（1/10） | 10%（1/10） |

![task 9 消融成功率](assets/figures/task9_ablation_success_rate.png)

该实验的可复现结论是：严格任务匹配的少样本 SFT 在这项任务上带来最稳定增益；当前 PPO 预算没有与它叠加增益，反而发生明显策略退化。这是单任务、每窗口 10 条轨迹的观察，而不是“PPO 一般无效”的普遍结论。
